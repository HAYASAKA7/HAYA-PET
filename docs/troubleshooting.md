...
| AI client launches while the HAYA companion is unavailable | Expected graceful fallback: the provider command still starts. Claude gets no HAYA settings or watchers for that launch. Codex gets no HAYA session hook overrides or watchers. Native Codex therefore never loads HAYA hooks. Antigravity and generic clients install no hooks. Start the companion, then restart the wrapped session for live status. |
| An existing Codex session still shows `HAYA Pet live status` after choosing **Quit** in HAYA Pet | Known limitation: Codex snapshots session hook overrides when its process starts, so HAYA cannot remove them externally. The lightweight dispatcher exits without loading the reporter or sending state after IPC closes. Use Codex's `/hooks` browser to disable the HAYA entries in that process, or exit it and start native `codex`. |
...
| Pet vanished but the tray icon/process is still running | Fixed in 0.3.16 for real GPU/renderer crashes, improved in 0.3.21 for alive-but-unpaintable compositor surfaces, and hardened in 0.3.22 for local Electron profile/cache isolation. Update to the latest version and restart the companion. Check `overlay-crash.log` and `companion.log` under `%LOCALAPPDATA%\haya-pet\logs` on Windows or `~/.haya-pet/logs` on macOS/Linux. |
...
| Pet vanishes after another local npm/Electron dev app crashes | Fixed in 0.3.22 — update and restart the companion. The other app may still need its own fix, such as excluding locked Chrome profile folders from Vite/chokidar watches, but HAYA now uses dedicated Electron `userData`, `sessionData`, and crash-dump paths instead of sharing generic dev Electron runtime state. |
| Pet right-click menu remains open after clicking another app, or HAYA appears in the taskbar while the menu is open | Fixed in 0.3.27 — update and restart the companion. Windows now gives the existing tray icon ownership of the native menu and never makes the transparent overlay focusable. |
| Running an agent with HAYA Pet causes sustained CPU/GPU usage or makes the desktop lag | Fixed in 0.3.28 — update and restart the companion. Pet frames and working indicators now update at the pet's actual frame cadence instead of the monitor refresh rate, animation stops while hidden, and linked Codex sessions no longer rescan the full rollout archive every 700 ms. Existing wrapped agents keep their already-loaded watcher code until restarted. |
...
| Pet shows only **idle/lifecycle** while **Codex** works | Live status is opt-in: run `haya-pet hooks on` once (persisted), then `haya-pet run --client codex -- codex`; approve Codex's one-time *review hooks* prompt. HAYA passes session-only hook config to that wrapped process. `thinking`/`idle` come from hooks, `running_tool`/`editing_files` from a transcript watcher, and approval states from the `PermissionRequest` hook plus a guardian-review watcher. |
| Wrapped Codex fails with `Error loading config.toml`, `invalid type: string`, and `expected a sequence` in `hooks` | Fixed in 0.3.26. The Windows `.cmd` launcher was stripping backslashes from escaped quotes inside HAYA's TOML override. Update HAYA Pet and restart the wrapped Codex session. |
| **Codex** reaches the AI subscription/usage limit but the pet stays **thinking** | Fixed in 0.3.19 — update to the latest version and restart the wrapped Codex session. HAYA Pet now reads both Codex `token_count.rate_limits.rate_limit_reached_type` markers and provider-limit turns that end as empty `task_complete` records, then reports **interrupted** instead of leaving the spinner active; because that state is non-terminal, the session bubble stays visible. |
| **Codex** manual `/compact` finishes but the pet shows **model response failed** | Fixed in 0.3.20 — update to the latest version and restart the wrapped Codex session. Codex writes `context_compacted` followed by an empty `task_complete` for that compact turn; HAYA Pet now treats that pair as compact bookkeeping and leaves the `PostCompact` hook's **compacted/idle** state intact. |
| **Codex** asks to review HAYA hooks again after you already approved them | Update to the latest version and start one online HAYA-wrapped Codex session. HAYA migrates its old global/profile hooks to a stable session source, so Codex may ask for one final review. Later unchanged wrapped launches reuse that trust identity; native Codex launches load no HAYA hooks. |
...
| **Codex** live status didn't turn on with a custom `-p`/`--profile` | Fixed — HAYA Pet passes hook definitions through session `-c` flags without consuming the profile slot. Run `haya-pet hooks on`, restart the wrapped Codex session, and approve the one-time hook review if Codex asks. Later user `-c` flags remain authoritative. |
