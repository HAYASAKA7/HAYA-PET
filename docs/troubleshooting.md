...
| Pet shows only **idle/lifecycle** while **Codex** works | Live status is opt-in: run `haya-pet hooks on` once (persisted, global), then `haya-pet run --client codex -- codex`; approve Codex's one-time *review hooks* prompt. Note Codex coverage is partial today — `thinking`/`idle` work, but `running_tool`/`editing_files`/*waiting for approval* don't fire yet (upstream [openai/codex#16732](https://github.com/openai/codex/issues/16732)). |
