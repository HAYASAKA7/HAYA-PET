...
| **Codex** asks to review HAYA hooks again after you already approved them | Fixed in 0.3.17 — update to the latest version, then approve once more if Codex asks. HAYA Pet keeps the existing valid hook command in `$CODEX_HOME/hooks.json`, avoids unchanged rewrites, and mirrors Codex's trusted `hooks.json` hashes into the selected profile config so the same hooks stay trusted across default/profile launches. |
